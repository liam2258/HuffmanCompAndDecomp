...
// A helper function for treeReconstructor, based on the node and associated flag it links the nodes together. The tracker variable keeps note of the index during each recursive call
...
Node *treeReconstructor(std::vector<unsigned char> treePacket) {
  ...
  for (int i = 0; i <= treePacket.size() - 6; i += 6) {
    std::vector<unsigned char> valueBytes {treePacket[i + 1], treePacket[i + 2],
      treePacket[i + 3], treePacket[i + 4]};
    int value = byteToInt(valueBytes);
    nodes.push_back(new Node(treePacket[i], value));
...
Node *createHuffmanTree(std::vector<Node *> &nodes) {
  ...
  // node and make the less used node the leftr child and the more used node the
  // right child
